module.exports = function ({ api, models }) {
                ...
                await new Promise(async resolve => {
                    for (const checkttFile of checkttData) {
                        ...
                        for (const item of checktt.day) {
                            const userName = await Users.getNameUser(item.id) || 'SHAHADAT SAHU ';
                        ...
                        let checkttBody = '==SHAHADAT SAHU  ❤️==\n\n';
                    ...
                    if (day_now == 1) {
                        ...
                        for (const checkttFile of checkttData) {
                            ...
                            for (const item of checktt.week) {
                                const userName = await Users.getNameUser(item.id) || 'Sahu Hun Yar';
                                const itemToPush = item;
                                itemToPush.name = userName;
                                storage.push(itemToPush);
                            };
    ...
    (async function () {
        ...
        try {
            logger(global.getText('listen', 'startLoadEnvironment'), '[ SHAHADAT SAHU  ]');
            ...
            logger.loader(global.getText('listen', 'loadedEnvironmentUser')), logger(global.getText('listen', 'successLoadEnvironment'), '[ Script ]');
        } catch (error) {
            return logger.loader(global.getText('listen', 'failLoadEnvironment', error), 'error');
        }
    }());
    logger(`[ ${global.config.PREFIX} ] • ${(!global.config.BOTNAME) ? "" : global.config.BOTNAME}`, "[ SHAHADAT SAHU  ]");
    ...
    //FUNCTION WORKS AS IT'S NAME, CRE: ScriptU
    const monthToMSObj = {
        1: 31 * 24 * 60 * 60 * 1000,
        2: 28 * 24 * 60 * 60 * 1000,
        3: 31 * 24 * 60 * 60 * 1000,
